...
def curriculum(net, data_loader, train_optimizer, memory_loader, test_loader, temperature, k, batch_size, epochs, save_name_pre, pretrain_model):
    ...
    for epoch in range(1, epochs + 1):
        if args.pretrain_model_path == '':
            ...
            if args.mass_candidate in ["mass_candidate", "mass_candidate_replacement"]:
                new_batch_idx_list = sample_from_mass(pretrain_model, data_loader, epoch, args.batch_size, args.use_out_reorder, args.reorder_reverse, args.curriculum, args.mass_candidate, all_in_flag, args.random_last_3batch)
            else:
