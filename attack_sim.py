...
def get_dbindex_loss(net, x, labels, loss_type, reverse, my_transform, num_clusters, repeat_num, use_out_dbindex, use_sim, kmean_result, use_wholeset_centroid, use_mean_dbindex, flag_select_confidence, confidence_thre, temperature):
    ...
    if loss_type == 'DBindex_cluster_momentum_kmeans_wholeset':
        ...
        for num_cluster_idx in range(len(num_clusters)):
            ...
            if not use_sim:
                ...
                inter_class_sim = torch.sum(inter_class_sim_matrix, dim=1)
