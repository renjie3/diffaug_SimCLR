...
from attack6 import PGD, get_dbindex_loss
...
def train_dbindex_loss_pytorch_loader(net, data_loader, train_optimizer, memory_loader, test_loader, temperature, k, batch_size, epochs, save_name_pre, train_data, const_train_data):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if args.my_train_loader:
            ...
            flag_train_dbindex = True
            # dbindex_epoch = args.piermaro_restart_epoch + epoch - args.start_dbindex_loss_epoch
            # if dbindex_epoch >= 0:
            #     if dbindex_epoch % 15 < 5:
            #         flag_train_dbindex = True
                # if args.piermaro_whole_epoch =='':
                #     if epochs - epoch < 3:
                #         flag_train_dbindex = True
                # else:
                #     if int(args.piermaro_whole_epoch) - epoch - args.piermaro_restart_epoch < 3:
                #         flag_train_dbindex = True
                
                
            if args.curriculum == 'DBindex_cluster_momentum_kmeans_wholeset' and args.piermaro_restart_epoch + epoch >= args.start_dbindex_loss_epoch and flag_train_dbindex:
            ...
            train_bar = tqdm(data_loader)
            for pos_1, pos_2, pos_org, target in train_bar:
                if args.piermaro_restart_epoch + epoch >= args.start_dbindex_loss_epoch and flag_train_dbindex:
