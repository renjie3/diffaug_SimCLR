    ...
    if loss_type == 'DBindex_cluster_momentum_kmeans_wholeset':
        ...
        for num_cluster_idx in range(len(num_clusters)):
            ...
            if not use_sim:
                ...
                intra_class_dis = []
                c = kmean_result['centroids'][num_cluster_idx].shape[0]
                mask_class = torch.ones(c).cuda()
                for i in range(c):
                    idx_i = torch.where(cluster_label == i)[0]
                    # class_i = sample[idx_i, :]
                    ...
                    if class_i.shape[0] == 0:
                        class_center.append(nn.functional.normalize(torch.ones_like(kmean_result['centroids'][num_cluster_idx][i].detach()), p=2, dim=0))
                        mask_class[i] = 0
                        # class_i_center = nn.functional.normalize(torch.ones_like(kmean_result['centroids'][num_cluster_idx][i].detach()), p=2, dim=0)
                        # class_center.append(class_i_center)
                        continue
                    else:
                        # class_i_center = class_i.mean(dim=0)
                ...
                if len(class_center) <= 1:
                    continue
                class_center = torch.stack(class_center, dim=0)
                class_center_wholeset = kmean_result['centroids'][num_cluster_idx]
                # input('no')
                ...
                mask = (torch.ones_like(class_dis) - torch.eye(class_dis.shape[0], device=class_dis.device)).bool()
                class_dis = class_dis.masked_select(mask).view(class_dis.shape[0], -1)
                class_dis = torch.sum(class_dis, dim=1)
                # print(class_dis.shape)
                class_dis = class_dis.masked_select(mask_class.bool())
                # print(class_dis.shape)
                # print(class_dis)
                # input()

                # for k in range(len(intra_class_dis)):
                #     print(type(intra_class_dis[k]))

                intra_class_dis = torch.stack(intra_class_dis, dim=0)#.masked_select(mask_class.bool())

                cluster_DB_loss = (intra_class_dis / (class_dis + 0.0001)).mean()
